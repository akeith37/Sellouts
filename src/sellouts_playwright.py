...
async def main():
    async with async_playwright() as p:
        browser = await p.chromium.launch_persistent_context(
            ...
            headless=False,
            userDataDir="/app/playwright-profile", #Store cookies/session info
            args=[
                "--start-maximied",
                "--no-sandbox",
    ...
    await page.setExtraHTTPHeaders({
        "User-Agent": "Mozilla/5.0 (X11; Linux aarch64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/115 Safari/537.36"
    })
    await page.goto(TICKET_URL, wait_until='networkidle')
